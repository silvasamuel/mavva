...
def _github_error(method: str, path: str, response: httpx.Response) -> str:
    if response.status_code == 403 and method == "POST" and path.rstrip("/") == "/pulls":
        return (
            "O GitHub recusou criar o pull request (403). "
            "No PAT fine-grained, ative Pull requests: Read and write "
            "(pull-requests:write), além de Contents: Read and write, "
            "e atualize GITHUB_TOKEN no Render."
        )
